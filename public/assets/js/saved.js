$(document).ready(function() {
  ...
  var articleContainer = $(".article-saved-container");
  ...
  function initPage() {
    ...
    $.get("/api/articles/saved").then(function(data) {
  ...
  function renderNotesList(data) {
    ...
    console.log("renderNotesList", data.notes)
    console.log("renderNotesList", data.notes.length);
    var notesToRender = [];
    var currentNote;
    if (!data.notes.note.length) {
      ...
      for (var i = 0; i < data.notes.note.length; i++) {
        ...
        .text(data.notes.note[i].noteText)
        .append($("<button class='btn btn-danger note-delete'>x</button>"));
        // Store the note id on the delete button for easy access when trying to delete
        currentNote.children("button").attr("data-id", data.notes.note[i]._id);
